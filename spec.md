...
[load_profiles] -> [build_prompt] -> [generate_draft] -> [validate] --(fail)--> [repair] -> [validate]
                                                           ...
                                                           (pass)
                                                              v
                                                          [select_assets] -> [return]
```

- `load_profiles`: fetch sender + receiver `CompanyProfile` from SQLite by id (404 if either is missing — see §5, error cases).
- `build_prompt`: assemble system prompt containing the layout template's field constraints (word limits per section, required sections, tone guidance) + both company profiles + the user's creative brief (`prompt` field from the request) + an explicit grounding instruction ("only state claims supported by the provided company context; do not invent facts, statistics, or claims about either company").
...
- `validate`: pure Python, no LLM — check every section's word count against the template's `max_words`/`min_words`. Also check every declared image slot has an entry.
- `repair` (conditional edge, max 2 attempts): for each failing field only, send a targeted follow-up ("the 'body_intro' section is 143 words, the limit is 120 — rewrite it to fit, preserving the key claim") rather than regenerating the whole article. After 2 failed attempts, hard-truncate at the word boundary and flag `"truncated": true` on that field in the response — never silently exceed the contract, never loop forever.
- `select_assets`: for each `image_placeholders` slot, pick the best-matching extracted `Image` from sender/receiver — now using both `tag` **and** the vision subagent's `description` text for a better match (e.g. `hero` slot ← sender's `logo`-tagged image; a slot asking for "a photo showing the receiver's industry context" can match against `description` semantically, not just the coarse tag) — or fall back to a `"source_hint": "stock_query"` with a short search query the downstream team could use.
